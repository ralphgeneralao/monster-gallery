...
@import '@react-md/overlay/dist/scss/mixins'; // have to import mixins since base styles are required
...
@mixin rmd-dialog {
  ...
  @if mixin-exists(rmd-app-bar-theme-update-var) {
    @include rmd-app-bar-theme-update-var(color, rmd-theme-var(text-primary-on-background));
  }
...
@mixin rmd-dialog-container {
  ...
  margin: rmd-dialog-theme-var(vertical-margin) rmd-dialog-theme-var(horizontal-margin);
  pointer-events: none;
}
