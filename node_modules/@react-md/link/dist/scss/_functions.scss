...
/// This function is used to quickly get one of the link's theme values. This is
/// really just for the `rmd-link-theme` mixin to provide some validation that a
/// correct style key is used, but might be useful in other cases.
///
/// @param {String} theme-style - One of the `$rmd-link-theme-values` map keys
/// to get a value for.
/// @return {Color|String|Number} one of the link's theme values.
@function rmd-link-theme($theme-style) {
  @return rmd-theme-get-var-value($theme-style, $rmd-link-theme-values, link);
}

/// This function is used to get one of the link's theme variables as a CSS
/// Variable to be applied as a style attribute. By default, the CSS Variable
/// will have a fallback of the current `$rmd-link-theme-values`
///
